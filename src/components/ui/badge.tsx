...
interface BadgeProps {
    ...
    variant?: 'defaut' | 'success' | 'warning' | 'danger' | 'info' | 'outline' | 'secondary';
    className?: string;
}

export const Badge = ({ children, variant = 'defaut', className }: BadgeProps) => {
    return (
        <span className={cn(
            "inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2",
            {
                'bg-gray-100 text-gray-800 dark:bg-gray-800 dark:text-gray-200': variant === 'defaut', // Default fallback
