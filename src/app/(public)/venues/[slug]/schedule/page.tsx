...
// Mock data
const mockCourts = [
    { id: 'c1', name: 'Sân A', type: 'Thảm BWF' },
    { id: 'c2', name: 'Sân B', type: 'Thảm BWF' },
    { id: 'c3', name: 'Sân C', type: 'Sàn Vina' },
    { id: 'c4', name: 'Sân D', type: 'Sàn Vina' },
    { id: 'c5', name: 'Sân E', type: 'VIP' },
    { id: 'c6', name: 'Sân F', type: 'Thường' },
    { id: 'c7', name: 'Sân G', type: 'Thường' },
    { id: 'c8', name: 'Sân H', type: 'Thường' }
];

// Thời gian từ 06:00 đến 22:00, mỗi slot 30ph
...
// Giả lập trạng thái đã bị đặt cho một số ô ngẫu nhiên
const isBooked = (courtId: string, time: string) => {
    const hash = courtId.charCodeAt(1) + time.charCodeAt(0) + time.charCodeAt(1);
    return hash % 7 === 0; // random booked
};
...
export default function VenueGlobalSchedulePage({ params }: { params: { slug: string } }) {
    ...
    const toggleSlot = (courtId: string, time: string) => {
        ...
        const exists = selectedSlots.find(s => s.court === courtId && s.time === time);
        if (exists) {
            setSelectedSlots(prev => prev.filter(s => !(s.court === courtId && s.time === time)));
        } else {
            setSelectedSlots(prev => [...prev, { court: courtId, time }]);
        }
    };
            ...
            <header className="bg-white border-b border-slate-200 sticky top-0 md:top-16 z-40 shadow-sm">
                <div className="container mx-auto px-4 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-xl md:text-2xl font-black text-slate-900 tracking-tight">Sân Cầu Lông Thống Nhất</h1>
                            <p className="text-xs font-semibold text-slate-500 flex items-center gap-1"><MapPin className="w-3.5 h-3.5" /> 138 Đào Duy Anh, P.9, Phú Nhuận</p>
                        </div>
                    </div>
                    <div className="hidden md:flex items-center gap-2 bg-slate-50 p-1.5 rounded-2xl border border-slate-100">
                        {['Hôm nay', 'Ngày mai', 'Thứ 6, 18/10', 'Thứ 7, 19/10'].map(d => (
                            <button 
                                key={d}
                                className={cn("px-4 py-2 font-bold text-sm rounded-xl transition-all shrink-0", selectedDate === d ? "bg-primary text-white shadow-sm" : "text-slate-500 hover:bg-slate-200/50 hover:text-slate-800")}
                ...
                <div className="bg-white rounded-[2rem] shadow-xl border border-slate-200/60 overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex flex-wrap gap-6 items-center justify-between text-sm">
                        <div className="flex items-center gap-2 font-bold text-slate-700">
                            <CalendarIcon className="w-4 h-4 text-primary" /> Lịch Trống Bao Quát - {selectedDate}
                        </div>
                        <div className="flex items-center gap-4">
                                    ...
                                    <div key={court.id} className="flex group hover:bg-primary/5 transition-colors">
                                        ...
                                        <div className="flex-1 flex">
                                            {timeSlots.map((time, idx) => {
                                                ...
                                                const selected = selectedSlots.some(s => s.court === court.id && s.time === time);
                                                ...
                                                return (
                                                    <div 
                                                        ...
                                                        className={cn(
                                                            "flex-1 min-w-[60px] border-r border-slate-50 p-1 flex items-center justify-center transition-all cursor-pointer",
                                                            booked 
                                                                ? "bg-rose-50/80 cursor-not-allowed" 
                                                                : selected 
                                                    ...
                                                    >
                                                        {selected ? (
                                                            ...
                                                            <div className="w-full h-full bg-rose-200/40 rounded border border-rose-300/60 striped-bg"></div>
                                                        ) : (
                    ...
                    <div className="container mx-auto max-w-7xl flex flex-col md:flex-row items-center justify-between gap-4">
                        ...
                        <div className="flex w-full md:w-auto items-center gap-6">
                            <div className="text-right hidden sm:block">
                                ...
                                <p className="text-2xl font-black text-primary">{(selectedSlots.length * 75000).toLocaleString('vi-VN')} ₫</p>
                            </div>
                            <Button 
                                size="lg" 
